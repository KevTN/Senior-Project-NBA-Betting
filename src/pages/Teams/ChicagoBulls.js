...
const ChicagoBulls = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const date = new Date();
      const teamStats = collection(db, 'nbaGames', String(date.getMonth() + 1), 'CHI');
      const teamRoster = collection(db, 'nbaRoster', String(date.getMonth() + 1), 'CHI');
  ...
  return (
    <div>
      ...
      <header className="bulls-header">
        <img src={logo} alt="Chicago Bulls Logo" className="bulls-logo" />
