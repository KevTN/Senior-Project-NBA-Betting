...
function NewYorkKnicks() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const date = new Date();
      const teamStats = collection(db, 'nbaGames', String(date.getMonth() + 1), 'NYK');
      const teamRoster = collection(db, 'nbaRoster', String(date.getMonth() + 1), 'NYK');
  ...
  return (
    <div>
      ...
      <header className="knicks-header">
        <img src={logo} alt="New York Knicks Logo" className="knicks-logo" />
        <h1 className="knicks-name">New York Knicks</h1>
      </header>
      <Sidebar />
