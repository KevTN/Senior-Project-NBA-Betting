...
function MiamiHeat() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const date = new Date();
      const teamStats = collection(db, 'nbaGames', String(date.getMonth() + 1), 'MIA');
      const teamRoster = collection(db, 'nbaRoster', String(date.getMonth() + 1), 'MIA');
  ...
  return (
    <div>
      ...
      <header className="heat-header">
        <img src={logo} alt="Miami Heat Logo" className="heat-logo" />
