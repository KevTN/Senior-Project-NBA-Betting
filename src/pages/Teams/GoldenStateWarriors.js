...
function GoldenStateWarriors() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const date = new Date();
      const teamStats = collection(db, 'nbaGames', String(date.getMonth() + 1), 'GSW');
      const teamRoster = collection(db, 'nbaRoster', String(date.getMonth() + 1), 'GSW');
  ...
  return (
    <div>
      ...
      <header className="warriors-header">
        <img src={logo} alt="Golden State Warriors Logo" className="warriors-logo" />
