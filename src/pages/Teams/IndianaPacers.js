...
function IndianaPacers() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const date = new Date();
      const teamStats = collection(db, 'nbaGames', String(date.getMonth() + 1), 'IND');
      const teamRoster = collection(db, 'nbaRoster', String(date.getMonth() + 1), 'IND');
  ...
  return (
    <div>
      ...
      <header className="pacers-header">
        <img src={logo} alt="Indiana Pacers Logo" className="pacers-logo" />
