...
function MemphisGrizzlies() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const date = new Date();
      const teamStats = collection(db, 'nbaGames', String(date.getMonth() + 1), 'MEM');
      const teamRoster = collection(db, 'nbaRoster', String(date.getMonth() + 1), 'MEM');
  ...
  return (
    <div>
      ...
      <header className="grizzlies-header">
        ...
        <h1 className="grizzlies-name">Memphis Grizzlies</h1>
      </header>
      <Sidebar />
